...
def test_integration_asap2_demo_v171(monkeypatch, compare_files, check_files_exist):
    """
    ...
    with tempfile.TemporaryDirectory(dir=temp_test_output_path, prefix=temp_test_dir_prefix) as tempdir:
        ...
        check_files_exist(
            f"{Path(tempdir).as_posix()}/ASAP2_Demo_V171.xml",
            f"{Path(tempdir).as_posix()}/ASAP2_Demo_V171.json",
            f"{Path(tempdir).as_posix()}/ASAP2_Demo_V171.yml",
        )

        # Compare files
        compare_files(
            f"{Path(tempdir).as_posix()}/ASAP2_Demo_V171.xml",
            f"{temp_test_output_path.as_posix()}/XML/ASAP2_Demo_V171.xml",
        )
        compare_files(
            f"{Path(tempdir).as_posix()}/ASAP2_Demo_V171.json",
            f"{temp_test_output_path.as_posix()}/JSON/ASAP2_Demo_V171.json",
        )
        compare_files(
            f"{Path(tempdir)}/ASAP2_Demo_V171.yml",
            f"{temp_test_output_path.as_posix()}/YAML/ASAP2_Demo_V171.yml",
        )
