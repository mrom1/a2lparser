...
import a2lparser.a2l.ast.a2l_ast as A2l_ast
...
class A2lYacc(object):
    def __init__(self, config: Config):
        ...
        self.a2l_lex = A2LLex(optimize=config.optimize, lextab=config.lex_tab, outputdir=config.gen_dir)
        # self.a2lLex.build_lexer(optimize=config.optimize, lextab=config.lex_tab, outputdir=config.gen_dir)
    ...
    def parse(self, input_string, filename, filelength, start_of_a2ml, end_of_a2ml):
        ...
        self.a2l_lex.start_of_a2ml = start_of_a2ml
        self.a2l_lex.end_of_a2ml = end_of_a2ml

        return self.a2lYacc.parse(input=input_string, lexer=self.a2l_lex, debug=self.debug)

    def generate_ast(self, input_string: str, last_line_no: int) -> AbstractSyntaxTree:
        """
        ...
        self.file_len = last_line_no
        self.a2l_lex.start_of_a2ml = 0
        self.a2l_lex.end_of_a2ml = 0

        ast = self.a2lYacc.parse(input=input_string, lexer=self.a2l_lex, debug=self.debug)
        return AbstractSyntaxTree(ast)
    ...
    def p_a2l_final_ext(self, p):
        ...
        """
        self.__update_progress(int(float(self.a2l_lex.get_line_number()) / float(self.file_len)) * 100)
        if p[2] and p[1]:
            p[1].extend(p[2])
        p[0] = p[1]
    ...
    def p_ANNOTATION_opt(self, p):
        ...
        if p[1]:
            ...
            self.__add_AST_Node_Param(
                node, [A2l_ast.Annotation_Label, A2l_ast.Annotation_Origin, A2l_ast.Annotation_Text], p[1]
            )
            p[0] = node
    ...
    def p_AXIS_DESCR_opt_objects(self, p):
        ...
        """
        node = self.__get_or_create_AST_Node(A2l_ast.Axis_Descr_Opt)
        self.__add_AST_Node_Object(
            NodeClass=node,
            AstNodeNames=[A2l_ast.Extended_Limits, A2l_ast.Fix_Axis_Par, A2l_ast.Fix_Axis_Par_Dist, A2l_ast.Fix_Axis_Par_List],
            Param=p[1],
        )
        p[0] = node
    ...
    def p_AXIS_DESCR_opt_objects_list(self, p):
        ...
        """
        node = self.__get_or_create_AST_Node(A2l_ast.Axis_Descr_Opt)
        self.__add_AST_Node_Object_List(NodeClass=node, AstNodeNames=[A2l_ast.Annotation], Param=p[1])
        p[0] = node
    ...
    def p_AXIS_PTS_opt_objects(self, p):
        ...
        node = self.__get_or_create_AST_Node(A2l_ast.Axis_Pts_Opt)
        self.__add_AST_Node_Object(
            NodeClass=node, AstNodeNames=[A2l_ast.Extended_Limits, A2l_ast.Symbol_Link, A2l_ast.Function_List], Param=p[1]
        )
        p[0] = node
    ...
    def p_AXIS_PTS_opt_objects_list(self, p):
        ...
        """
        node = self.__get_or_create_AST_Node(A2l_ast.Axis_Pts_Opt)
        self.__add_AST_Node_Object_List(NodeClass=node, AstNodeNames=[A2l_ast.Annotation, A2l_ast.If_Data], Param=p[1])
        p[0] = node
    ...
    def p_CALIBRATION_HANDLE_opt(self, p):
        ...
        self.__add_AST_Node_Param(NodeClass=node, AstNodeNames=[A2l_ast.Calibration_Handle_Text], Param=p[1])
        p[0] = node
    ...
    def p_CALIBRATION_METHOD(self, p):
        ...
        if len(p) > 2:
            if len(p) == 7:
                p[0] = A2l_ast.Calibration_Method(Method=p[3], Version=p[4])
            else:
                p[0] = A2l_ast.Calibration_Method(Method=p[3], Version=p[4], Calibration_Handle=p[5])
    ...
    def p_CHARACTERISTIC_opt_objects(self, p):
        ...
        self.__add_AST_Node_Object(
            NodeClass=node,
            AstNodeNames=[
                A2l_ast.Dependent_Characteristic,
    ...
    def p_CHARACTERISTIC_opt_objects_list(self, p):
        ...
        """
        node = self.__get_or_create_AST_Node(A2l_ast.Characteristic_Opt)
        self.__add_AST_Node_Object_List(
            NodeClass=node, AstNodeNames=[A2l_ast.Annotation, A2l_ast.Axis_Descr, A2l_ast.If_Data], Param=p[1]
        )
        p[0] = node
    ...
    def p_COEFFS(self, p):
        ...
        # INT = f(PHYS)
        p[0] = A2l_ast.Coeffs(a=p[2], b=p[3], c=p[4], d=p[5], e=p[6], f=p[7])
    ...
    def p_COEFFS_LINEAR(self, p):
        ...
        # f(x) = ax + b
        # PHYS = f(INT)
        p[0] = A2l_ast.Coeffs_Linear(a=p[2], b=p[3])
    ...
    def p_COMPU_METHOD(self, p):
        ...
        """
        node = self.__create_AST_Node(
            A2l_ast.Compu_Method(Name=p[3], LongIdentifier=p[4], ConversionType=p[5], Format=p[6], Unit=p[7])
        )
    ...
    def p_COMPU_METHOD_opt_params(self, p):
        ...
        self.__add_AST_Node_Param(
            NodeClass=node, AstNodeNames=[A2l_ast.Compu_Tab_Ref, A2l_ast.Ref_Unit, A2l_ast.Status_String_Ref], Param=p[1]
        )
    ...
    def p_COMPU_TAB(self, p):
        ...
        """
        node = self.__create_AST_Node(
            A2l_ast.Compu_Tab(Name=p[3], LongIdentifier=p[4], ConversionType=p[5], NumberValuePairs=p[6], Axis_Points=p[7])
        )
    ...
    def p_COMPU_VTAB(self, p):
        ...
        """
        p[0] = A2l_ast.Compu_Vtab(Name=p[3], LongIdentifier=p[4], ConversionType=p[5], NumberValuePairs=p[6], InVal_OutVal=p[7])
        if len(p) == 11:
    ...
    def p_COMPU_VTAB_RANGE(self, p):
        ...
        """
        p[0] = A2l_ast.Compu_Vtab_Range(Name=p[3], LongIdentifier=p[4], NumberValueTriples=p[5], InVal_MinMax_OutVal=p[6])
        if len(p) == 10:
    ...
    def p_DEPENDENT_CHARACTERISTIC(self, p):
        ...
        """
        p[0] = A2l_ast.Dependent_Characteristic(Formula=p[3], Characteristic=p[4])
    ...
    def p_EXTENDED_LIMITS(self, p):
        ...
        """
        p[0] = A2l_ast.Extended_Limits(LowerLimit=p[2], UpperLimit=p[3])

    def p_FIX_AXIS_PAR(self, p):
        """
        fix_axis_par     : FIX_AXIS_PAR constant constant constant
        """
        p[0] = A2l_ast.Fix_Axis_Par(Offset=p[2], Shift=p[3], Numberapo=p[4])

    def p_FIX_AXIS_PAR_DIST(self, p):
        """
        fix_axis_par_dist     : FIX_AXIS_PAR_DIST constant constant constant
        """
        p[0] = A2l_ast.Fix_Axis_Par_Dist(Offset=p[2], Distance=p[3], Numberapo=p[4])
    ...
    def p_FNC_VALUES(self, p):
        ...
        """
        p[0] = A2l_ast.Fnc_Values(Position=p[2], Datatype=p[3], IndexMode=p[4], AddressType=p[5])
    ...
    def p_FORMULA(self, p):
        ...
        if len(p) == 6:
            ...
            p[0] = A2l_ast.Formula(f_x=p[3], Formula_Inv=p[4])
    ...
    def p_FRAME(self, p):
        ...
        """
        node = self.__create_AST_Node(A2l_ast.Frame(Name=p[3], LongIdentifier=p[4], ScalingUnit=p[5], Rate=p[6]))
    ...
    def p_FRAME_opt_params(self, p):
        ...
        self.__add_AST_Node_Param(NodeClass=node, AstNodeNames=[A2l_ast.Frame_Measurement], Param=p[1])
        p[0] = node
    ...
    def p_FRAME_opt_objects_list(self, p):
        ...
        """
        node = self.__get_or_create_AST_Node(A2l_ast.Frame_Opt)
        self.__add_AST_Node_Object_List(NodeClass=node, AstNodeNames=[A2l_ast.If_Data], Param=p[1])
        p[0] = node
    ...
    def p_FUNCTION(self, p):
        ...
        """
        node = self.__create_AST_Node(A2l_ast.Function(Name=p[3], LongIdentifier=p[4]))
    ...
    def p_FUNCTION_opt_params(self, p):
        ...
        self.__add_AST_Node_Param(NodeClass=node, AstNodeNames=[A2l_ast.Function_Version], Param=p[1])
        p[0] = node
    ...
    def p_FUNCTION_opt_objects_list(self, p):
        ...
        node = self.__get_or_create_AST_Node(A2l_ast.Function_Opt)
        self.__add_AST_Node_Object_List(
            NodeClass=node, AstNodeNames=[A2l_ast.Annotation, A2l_ast.If_Data, A2l_ast.Sub_Function], Param=p[1]
        )
        p[0] = node
    ...
    def p_GROUP(self, p):
        ...
        """
        node = self.__create_AST_Node(A2l_ast.Group(GroupName=p[3], GroupLongIdentifier=p[4]))
    ...
    def p_GROUP_opt_params(self, p):
        ...
        self.__add_AST_Node_Param(NodeClass=node, AstNodeNames=[A2l_ast.Root], Param=p[1])
    ...
    def p_GROUP_opt_objects(self, p):
        ...
        self.__add_AST_Node_Object(
            NodeClass=node,
            AstNodeNames=[A2l_ast.Function_List, A2l_ast.Ref_Characteristic, A2l_ast.Ref_Measurement, A2l_ast.Sub_Group],
            Param=p[1],
        )
        p[0] = node
    ...
    def p_HEADER_opt(self, p):
        ...
        self.__add_AST_Node_Param(NodeClass=node, AstNodeNames=[A2l_ast.Project_No, A2l_ast.Version], Param=p[1])
    ...
    def p_IDENTIFICATION(self, p):
        ...
        """
        p[0] = A2l_ast.Identification(Position=p[2], Datatype=p[3])
    ...
    def p_IF_DATA(self, p):
        ...
        p[0] = A2l_ast.If_Data(
            Name=p[1], OptionalParams=A2l_ast.If_Data_Opt(DataParams=data_params, If_Data_Block=if_data_block)
        )
    ...
    def p_IF_DATA_block(self, p):
        ...
        if_data_block = [x for x in p[2] if isinstance(x, A2l_ast.If_Data_Block)]
        data_params = [x for x in p[2] if not isinstance(x, A2l_ast.If_Data_Block)]
        p[0] = A2l_ast.If_Data_Block(Name=p[1], DataParams=data_params, If_Data_Block=if_data_block)
    ...
    def p_MATRIX_DIM(self, p):
        ...
        """
        p[0] = A2l_ast.Matrix_Dim(xDim=p[2], yDim=p[3], zDim=p[4])
    ...
    def p_MAX_REFRESH(self, p):
        ...
        """
        p[0] = A2l_ast.Max_Refresh(ScalingUnit=p[2], Rate=p[3])
    ...
    def p_MEMORY_LAYOUT(self, p):
        ...
        """
        p[0] = A2l_ast.Memory_Layout(PrgType=p[3], Address=p[4], Size=p[5], Offset=p[6])
        if len(p) == 10:
            p[0].If_Data = p[7]
    ...
    def p_MODULE(self, p):
        ...
        """
        if len(p) == 5:
            node = self.__create_AST_Node(A2l_ast.Module(Name=None, LongIdentifier=None))
        else:
            node = self.__create_AST_Node(A2l_ast.Module(Name=p[3], LongIdentifier=p[4]))
    ...
    def p_NO_RESCALE_X(self, p):
        ...
        """
        p[0] = A2l_ast.No_Rescale_X(Position=p[2], Datatype=p[3])
    ...
    def p_OFFSET_X(self, p):
        ...
        """
        p[0] = A2l_ast.Offset_X(Position=p[2], Datatype=p[3])

    def p_OFFSET_Y(self, p):
        """
        offset_y     : OFFSET_Y constant datatype
        """
        p[0] = A2l_ast.Offset_Y(Position=p[2], Datatype=p[3])

    def p_OFFSET_Z(self, p):
        """
        offset_z     : OFFSET_Z constant datatype
        """
        p[0] = A2l_ast.Offset_Z(Position=p[2], Datatype=p[3])

    def p_OFFSET_Z4(self, p):
        """
        offset_z4     : OFFSET_Z4 constant datatype
        """
        p[0] = A2l_ast.Offset_Z4(Position=p[2], Datatype=p[3])

    def p_OFFSET_Z5(self, p):
        """
        offset_z5     : OFFSET_Z5 constant datatype
        """
        p[0] = A2l_ast.Offset_Z5(Position=p[2], Datatype=p[3])
    ...
    def p_PROJECT(self, p):
        ...
        """
        node = self.__create_AST_Node(A2l_ast.Project(Name=p[3], LongIdentifier=p[4]))
    ...
    def p_PROJECT_opt_objects(self, p):
        ...
        self.__add_AST_Node_Object(NodeClass=node, AstNodeNames=[A2l_ast.Header], Param=p[1])
        p[0] = node
    ...
    def p_PROJECT_opt_objects_list(self, p):
        ...
        """
        node = self.__get_or_create_AST_Node(A2l_ast.Project_Opt)
        self.__add_AST_Node_Object_List(NodeClass=node, AstNodeNames=[A2l_ast.Module], Param=p[1])
        p[0] = node
    ...
    def p_RECORD_LAYOUT_opt_objects_list(self, p):
        ...
        """
        node = self.__get_or_create_AST_Node(A2l_ast.Record_Layout_Opt)
        self.__add_AST_Node_Object_List(NodeClass=node, AstNodeNames=[A2l_ast.Reserved], Param=p[1])
        p[0] = node
    ...
    def p_RESERVED(self, p):
        ...
        """
        p[0] = A2l_ast.Reserved(Position=p[2], DataSize=p[3])
    ...
    def p_SYMBOL_LINK(self, p):
        ...
        """
        p[0] = A2l_ast.Symbol_Link(SymbolName=p[2], Offset=p[3])

    def p_SYSTEM_CONSTANT(self, p):
        """
        system_constant     : SYSTEM_CONSTANT string_literal string_literal
        """
        p[0] = A2l_ast.System_Constant(Name=p[2], Value=p[3])
    ...
    def p_UNIT(self, p):
        ...
        """
        node = self.__create_AST_Node(A2l_ast.Unit(Name=p[3], LongIdentifier=p[4], Display=p[5], Type=p[6]))
    ...
    def p_UNIT_opt_params(self, p):
        ...
        self.__add_AST_Node_Param(NodeClass=node, AstNodeNames=[A2l_ast.Ref_Unit], Param=p[1])
    ...
    def p_UNIT_opt_objects(self, p):
        ...
        """
        node = self.__get_or_create_AST_Node(A2l_ast.Unit_Opt)
        self.__add_AST_Node_Object(NodeClass=node, AstNodeNames=[A2l_ast.Si_Exponents, A2l_ast.Unit_Conversion], Param=p[1])
        p[0] = node
    ...
    def p_UNIT_CONVERSION(self, p):
        ...
        """
        p[0] = A2l_ast.Unit_Conversion(Gradient=p[2], Offset=p[3])
    ...
    def p_USER_RIGHTS_opt_params(self, p):
        ...
        self.__add_AST_Node_Param(NodeClass=node, AstNodeNames=[A2l_ast.Read_Only], Param=p[1])
        p[0] = node
    ...
    def p_USER_RIGHTS_opt_objects_list(self, p):
        ...
        """
        node = self.__get_or_create_AST_Node(A2l_ast.User_Rights_Opt)
        self.__add_AST_Node_Object_List(NodeClass=node, AstNodeNames=[A2l_ast.Ref_Group], Param=p[1])
        p[0] = node
    ...
    def p_VAR_CHARACTERISTIC(self, p):
        ...
        """
        if len(p) == 7:
            p[0] = A2l_ast.Var_Characteristic(Name=p[3], CriterionName=p[4])
        else:
            p[0] = A2l_ast.Var_Characteristic(Name=p[3], CriterionName=p[4], Var_Address=p[5])
    ...
    def p_VAR_CRITERION(self, p):
        ...
        """
        node = self.__create_AST_Node(A2l_ast.Var_Criterion(Name=p[3], LongIdentifier=p[4], Value=p[5]))
    ...
    def p_VAR_CRITERION_opt(self, p):
        ...
        self.__add_AST_Node_Param(
            NodeClass=node, AstNodeNames=[A2l_ast.Var_Measurement, A2l_ast.Var_Selection_Characteristic], Param=p[1]
        )
    ...
    def p_VARIANT_CODING_opt_params(self, p):
        ...
        self.__add_AST_Node_Param(NodeClass=node, AstNodeNames=[A2l_ast.Var_Naming, A2l_ast.Var_Separator], Param=p[1])
    ...
    def p_VARIANT_CODING_opt_objects_list(self, p):
        ...
        self.__add_AST_Node_Object_List(
            NodeClass=node,
            AstNodeNames=[A2l_ast.Var_Characteristic, A2l_ast.Var_Criterion, A2l_ast.Var_Forbidden_Comb],
            Param=p[1],
        )
        p[0] = node
    ...
    def p_VIRTUAL_CHARACTERISTIC(self, p):
        ...
        """
        p[0] = A2l_ast.Virtual_Characteristic(Formula=p[3], Characteristic=p[4])
